...
export const MetaConnect = () => {
  ...
  return (
    <button
      onClick={handleLogin}
      disabled={loading}
      className="flex items-center gap-2 bg-[#1877F2] hover:bg-[#166fe5] text-white px-4 py-2 rounded-full font-medium transition-all shadow-lg hover:shadow-blue-500/20 disabled:opacity-70 disabled:cursor-not-allowed text-sm"
    >
      <Facebook size={16} />
      {loading ? 'Conectando...' : 'Conectar Business'}
    </button>
  );
};
